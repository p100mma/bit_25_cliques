...
* hierarchy of clique partitions and coupled meaningful separation thresholds. 
...
Conclusion: the range of `minimum_cluster_size` for which the result is correct for LDF is much wider, although some sharp transitions are noticeable. 
For HDBSCAN*, the progression is much more gradual.
...
**Output:** "clique consensus similarities" ($CCS_{ij}$) between all pairs of genes $i,j$ 
...
$$
\widehat{cs}(i,j) = \frac{1}{B} \sum_{b=1}^{B} cs_{b}(i,j)
$$
...
$$
x_o = \{(v_1, c_1)_1, (v_2, c_2)_2, \dots, (v_p, c_p)_p\}
$$

where $c_j$ is the identifier of the clique to which node $j$ belongs.

Now, consider the space of solutions found from all possible runs of such algorithm on a graph on which the nodes stay fixed, but weights can vary. 
...
Where $P= \left \{C_1, C_2 \dots \right \}$ is a clique partition of the graph (including singleton cliques).
...
We have to keep in mind that this is biased, because the weights could vary in such a way that some of the possible configurations for a fixed $P$ and $t$ are less probable. 
But this aspect of variability which we neglect is captured by the "consensus" part - such partition is biased for one, specific run, but the image from the clique similarities averaged over multiple runs shows a more generalized picture.
...
Preliminary comparison shows that sequential algorithm is more stable, thus a better candidate for CCS than complete linkage.
...
DynamicTreeCut has a vast number of parameters, but we use the default values, except for `minClusterSize`, which we fine tune using a two-fold strategy:
...
- objective function evaluation - we opt for modularity - as conductance showed negligible variability across the range of `minClusterSize` values.
...
For DTC, in principle, we should pick parameters separately for each of the similarity functions. 
We do this here, all on the "basic" similarity network - either computed on original data (for $TO$ measure) or first of 30 resamples (for $CS$ and $CCS$ measures).
...
ggheat<- function(array,line_ys,main=NULL){
  reshape2::melt(array)-> flat_array
...
for (metric in names(simXmcs)){
  ...
  sub_df[, grep("MOD", colnames(sub_df))]=sub_df[, grep("MOD", colnames(sub_df))]*20 #for scaling on the plot 
  print(ggheat(array=ari_mats[[metric]], line_ys = sub_df, main=metric))
}
mcs_values<- c(mcs_range[[7]], mcs_range[[16]])

```
Above plots suggest that 160 is also a good choice for the derived similarity measures. 
...
if (!file.exists("performance_dtc.rds")){
...
saveRDS(performance_array, "performance_dtc.rds")
} else {
 performance_array<- readRDS("performance_dtc.rds")
}
...
##### Module-trait association investigation

How well each gene grouping matches distinct tissue types?
...
For each repetition, we measure such "matching" by computing association of each type to each module by usage of $MI$-test (between binary indicator of a subtype and eigengene of each module).

We want to count how many times, the best fitting module for each type is the best fitting type for that module.
One catch here is that we normalize the association strength type-wise.
This does not change the preference of the type but changes the best fitting type for each module. 
Some of the types are easily discernible by majority of the modules, this normalization mitigates that.

Steps are as follows:

1. Compute association of each of the 5 subtypes $Y_i$ to each module eigengene $E_k$ by $MI$ test= $A_{ik}= MI(E_k,Y_i) \times [p.value_{ik}<0.05]$

2. Normalise: $\widehat{A_{ik}}= A_{ik}/\sum_l A_{il}$ 

Compute preferences: 

$Y_{pref}(i) = \arg max_{k} \widehat{A_{ik}}$

$E_{pref}(k) = \arg max_{i} \widehat{A_{ik}}$

Matching score for one repetition of the experiment is then:

$$
\sum_i \left [ Y_{pref}(i) = E_{pref} ( Y_{pref}(i) ) \right ]
$$
(we count the number of times expression $[]$ was true with $i$ ranging over the types).


```{r}

# setting up the data structures, merging results from both algorithms to one array
...
```{r, message=FALSE, warning=FALSE}
#setting up structures to hold the results of the association tests
#for each similarity and algorithm we can get a matrix of size 
# n_types x n_modules, n_modules can vary across methods and repeats
unique(y_char)-> types
